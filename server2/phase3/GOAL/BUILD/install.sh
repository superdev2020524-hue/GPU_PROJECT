...
# GOAL register structure:
#   BUILD/install.sh (this script)
#   SOURCE/ (C source files and headers)
#   INCLUDE/ (protocol headers)
SHIM_SRC_DIR="${SCRIPT_DIR}/../SOURCE"
INCLUDE_DIR="${SCRIPT_DIR}/../INCLUDE"

# Verify directories exist
if [[ ! -d "${SHIM_SRC_DIR}" ]]; then
    echo "ERROR: SOURCE directory not found: ${SHIM_SRC_DIR}" >&2
    exit 1
fi
...
build_shims() {
    ...
    if ! gcc -shared -fPIC -O2 -Wall -Wextra -std=c11 -D_GNU_SOURCE \
    ...
    if [[ ! -f "${SHIM_SRC_DIR}/libvgpu-cuda.so" ]]; then
        error "libvgpu-cuda.so was not created after build"
        return 1
    fi
    ...
    if ! gcc -shared -fPIC -O2 -Wall -Wextra -std=c11 -D_GNU_SOURCE \
        ...
        "${SHIM_SRC_DIR}/cuda_transport.c" \
        -lpthread -ldl; then
        error "Failed to build libvgpu-nvml.so"
        return 1
    fi
    if [[ ! -f "${SHIM_SRC_DIR}/libvgpu-nvml.so" ]]; then
        error "libvgpu-nvml.so was not created after build"
        return 1
    fi
