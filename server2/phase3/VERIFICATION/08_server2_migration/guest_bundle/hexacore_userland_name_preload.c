...
static int branding_enabled(void) {
    const char *brand_bypass = getenv("HEXACORE_BRAND_BYPASS");
    const char *smi_bypass = getenv("HEXACORE_NVIDIA_SMI_BYPASS");

    return !(
        (brand_bypass && strcmp(brand_bypass, "1") == 0) ||
        (smi_bypass && strcmp(smi_bypass, "1") == 0)
    );
}
