...
python3 connect_host.py "pkill -x mediator_phase3 2>/dev/null; sleep 2; cd /root/phase3 && nohup ./mediator_phase3 2>/tmp/mediator.log </dev/null & disown; sleep 2; pgrep -a mediator"
```

Or on the host itself (e.g. `ssh root@10.25.33.10`):

```bash
pkill -x mediator_phase3
sleep 2
