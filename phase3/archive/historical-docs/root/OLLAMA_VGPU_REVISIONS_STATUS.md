...
## Actual Oyu captured (Mar 16)

- **Script:** `capture_ollama_actual_error.py` — runs server in foreground with logging, triggers generate, reads log.
- **Result:** When the server starts **fresh**, discovery’s bootstrap runner (with only the vGPU visible) reports **0 devices**, so Ollama logs **“filtering device which didn’t fully initialize”** and falls back to CPU. So the **actual Oyu** in that scenario is: device filtered during bootstrap because `bootstrapDevices(...)` returned empty for our vGPU.
- **Detail:** See **ACTUAL_OLLAMA_ERROR_CAPTURED.md** for log excerpts, flow (runner.go second pass, bootstrapDevices), and next steps (mediator up before discovery, cuDeviceGetCount during bootstrap, service vs manual start).

## Summary and next steps (Mar 16)

- **CURRENT_STAGE_SUMMARY_AND_NEXT_STEPS.md** — How we reached the current stage; verification that the discovery issue (Issue A) is resolved; next steps to fix the post-GEMM crash (Issue B) and achieve Stage 1.

## References

- **ACTUAL_OLLAMA_ERROR_CAPTURED.md** — Captured server log and “actual Oyu” (filtering device which didn’t fully initialize).
